...
export class PracticeLayout {
  constructor(piano) {
    ...
    try { const saved = localStorage.getItem('atelier-practice-ratio'); if (saved !== null && Number.isFinite(Number(saved))) this.ratio = clamp(Number(saved), 0, .55); } catch {}
  ...
  setRatio(value) {
    if (this.piano.view.rows) { this.twoRowRatio = clamp(value, 0, .4); this.resize(); return; }
    this.ratio = clamp(value, 0, .55);
  ...
  resize() {
    ...
    const sheet = Math.round(clamp(this.usableHeight * ratio, 0, Math.max(110, this.usableHeight - controls - (twoRows ? 280 : 150))));
    const panel = document.getElementById('sheet-panel');
    panel.style.visibility = sheet === 0 ? 'hidden' : '';
    panel.inert = sheet === 0;
    ...
    toggle.textContent = this.twoRowRatio === 0 ? 'Show sheet' : this.twoRowRatio > .25 ? 'Collapse sheet' : 'Expand sheet';
