...
(async()=>{
  ...
  try {
    ...
    assert.equal(await page.locator('.white-key[data-row="0"]').count(),23);
    assert.equal(await page.locator('.white-key[data-row="1"]').count(),29);
    ...
    await page.locator('#practice-divider').focus();
    for(let i=0;i<20;i++) await page.keyboard.press('ArrowUp');
    assert.equal(await page.locator('#sheet-panel').isVisible(),false);
    await page.locator('#practice-divider').press('Home');
    assert.ok(await page.locator('#sheet-panel').isVisible());
    await page.locator('#keyboard-mode').selectOption('full');
    report.push('All four keyboard modes, C4 row split, sheet hiding/reopening and octave navigation pass in WebKit.');
